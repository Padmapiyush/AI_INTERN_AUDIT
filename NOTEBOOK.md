...
1. The fertility script splits on `" "` (single space) — what happens with double spaces? I noticed the corpus files have them.
...
While the corpus is downloading, I reviewed `fertility.py` line-by-line and found 4 specific issues:
...
3. **The Code Bug:** `line.split(" ")` breaks on double-spaces, creating empty words. Both `eng_sample.txt` and `hin_sample.txt` have a double-space typo!
...
I've written `fertility_corrected.py` to fix the first 3 bugs and support multiple denominators (words, chars, bytes, graphemes, sentences).
...
- Derived the KV cache limit: ~27 concurrent 4096-token sequences fits in the available 12.08 GB.
- Explained the throughput anomaly: batch 32+ exceeds the 27 sequence limit, causing KV cache preemptions and expensive recomputations.
